...
object KtorEngineMocked {
    ...
    @JvmStatic
    @JvmOverloads
    fun getJsonFromPath(@Language("http-url-reference") path: String, mockServer: Boolean = false) = when (path) {
        ...
        "/infos" -> readResourceFile("/api/responses/get_infos.json")

        // WARNING: offers can be old and user_token
        ...
        "/genre/0/podcasts" -> {
            KermitLogger.i("Json not have data")
            // TODO: find with data
        ...
        "/playlist/4341978/fans" -> readResourceFile("/api/responses/playlist/get_playlist_fans.json")
        ...
        // always empty?
        "/podcast/20269" ->
            readResourceFile("/api/responses/podcast/get_podcast_id.json")

        // special: available = false
        "/podcast/20269/episodes" ->
            readResourceFile("/api/responses/podcast/get_podcast_episodes.json")

        // special: data is empty and next not
        ...
        "/user/2616835602" -> readResourceFile("/api/responses/user/get_user_id.json")
            ...
            KermitLogger.e("Mock (Server $mockServer) request not mapped $path")
